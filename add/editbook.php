 ...
 $section = filter_var($section,FILTER_SANITIZE_NUMBER_INT);            
	...
	if (in_array($bookactualFileExt, $bookallowed)) {
   	...
   	$bookName = $book['location'];
		...
		$sql = "UPDATE book set name='$name',title='$title',image='$imgName',location='$bookName',subject='$subject',summary='$summary',author='$author',version='$version',date_create='$date_create',tag='$tag',keywords='$keywords',categories='$categories',section='$section',type_book='$pay',price='$price',status='$status',isbn='$isbn',book_format='$book_format',edition_language='$edition_language',publisher='$publisher' where id='$idsbook' ";
