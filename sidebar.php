<div class="col-lg-3 col-md-12 col-sm-12 col-xs-12">
	...
	<?php echo $ads['ads5'] ;
                            ...
                            <div class="widget">
                                <h2 class="widget-title"><?php echo $lang['trend_book'] ;?></h2>
  ...
  $sd = implode(" d ",$num);
	...
	$sql = "SELECT * FROM book  where view <= $sd AND status$logc'$status_post' ORDER BY view DESC limit $more_view_post";
  	...
  	?>
                                    <div class="blog-box">
                                        <div class="post-media">
                                            <a href="single.php?id=<?php echo $row['id'] ;?>&<?php echo $row['title'] ;?>" title="">
                                                <img src="upload/images/book/<?php echo $row['image'] ;?>" alt="" class="img-fluid" style="width:800px; height:460px;">
                                        ...
                                        <div class="blog-meta">
                                            <h4><a href="single.php?id=<?php echo $row['id'] ;?>&<?php echo $row['title'] ;?>" title=""><?php echo $row['name'] ;?></a></h4>
                                        </div><!-- end meta -->
                                    </div><!-- end blog-box -->
                                    	<hr class="invis">
  <?php
...
?>
  ...
  $sd = implode(" d ",$num);
	...
	$sql = "SELECT * FROM book  where download <= $sd AND status$logc'$status_post' ORDER BY download DESC limit $more_down_post";
$result = $conn->query($sql);
$siteData = mysqli_fetch_assoc($result);
if($result = mysqli_query($conn, $sql)){
            	    if(mysqli_num_rows($result) > 0){
...
/* if ($result->num_rows >= 0) {
  ...
  while($row = $result->fetch_assoc()) { */
  	?>
                                        ...
                                        <a href="single.php?id=<?php echo $row['id'] ;?>&<?php echo $row['title'] ;?>" class="list-group-item list-group-item-action flex-column align-items-start">
	...
	<?php }
        ?>                                  </div>
                            ...
                            <div class="widget">
                                ...
                                <div class="blog-list-widget">
                                    <div class="list-group">
                                    <?php	$sql = "SELECT * FROM book  where type_book='1' AND status$logc'$status_post' ORDER BY id DESC limit $view_paid";
$result = $conn->query($sql);
$siteData = mysqli_fetch_assoc($result);
if($result = mysqli_query($conn, $sql)){
            	    if(mysqli_num_rows($result) > 0){
