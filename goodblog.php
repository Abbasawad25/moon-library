  ...
  $sd = implode(" d ",$num);
	...
	$sql = "SELECT * FROM blog  where view <= $sd  AND status='$status' ORDER BY view DESC LIMIT $more_view_post";
        ...
        <div class="blog-box row">
                                    <div class="col-md-4">
                                        <div class="post-media">
                                            <a href="viewblog.php?id=<?php echo $row['id'] ;?>&title=<?php echo $row['title_blog'];?>" title="">
...
$idca = $row['cat_id'];
 ...
 ?>    
                                        <small class="firstsmall" dir="auto"><a class="bg-orange" href="category.php?cate=<?php echo $g['id'] ;?>" title=""><?php echo $g['name'] ;?></a></small>
                                        <small dir="auto"><a href="blog.php?tag=<?php echo $row['date'] ;?>" title=""><?php echo $row['date'] ;?></a></small>
                                  <? 
$iduser = $row['iduser'];
 ...
 ?>      <small dir="auto"><a href="author.php?id=<?php echo $row['iduser'] ;?>" title=""><?php echo $ro['username'] ;?></a></small>
                                        <small dir="auto"><a href="blog.php?tag=<?php echo $row['view'] ;?>" title=""><i class="fa fa-eye"></i><?php echo " " . $row['view'] . " ";?></a></small>
                                    </div><!-- end meta -->
                                </div><!-- end blog-box -->
<hr class="invis">
...
<hr class="invis">
  ...
  $sd = implode(" d ",$num);
	...
	$sql = "SELECT * FROM blog  where download <= $sd  AND status='$status' ORDER BY download DESC LIMIT $more_down_post";
$result = $conn->query($sql);
$siteData = mysqli_fetch_assoc($result);
if($result = mysqli_query($conn, $sql)){
            	    if(mysqli_num_rows($result) > 0){
...
$idca = $row['cat_id'];
 ...
 ?>    
                                        <small class="firstsmall" dir="auto"><a class="bg-orange" href="category.php?cate=<?php echo $g['id'] ;?>" title=""><?php echo $g['name'] ;?></a></small>
                                        <small dir="auto"><a href="blog.php?<?php echo $row['date'] ;?>" title=""><?php echo $row['date'] ;?></a></small>
                                  <? 
$iduser = $row['iduser'];
 $ro = mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM users where id='$iduser' ")); 
 ?>      <small dir="auto"><a href="author.php?id=<?php echo $row['iduser'] ;?>" title=""><?php echo $ro['username'] ;?></a></small>
